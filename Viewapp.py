...
# Dados iniciais (em memória)
# -------------------------
# padrão inicial (usado somente se não existir arquivos json)
_default_usuarios = [
    {"usuario": "admin", "senha": "0000", "tipo": "CORDENADOR"},
    {"usuario": "user", "senha": "1234", "tipo": "PROFESSOR"},
    {"usuario": "root", "senha": "9999", "tipo": "DIRETOR"}
...
def salvar_todos():
    # salvar usuarios, categorias, itens_exemplares, historico
    try:
        salvar_json(USUARIOS_FILE, usuarios)
        salvar_json(CATEGORIAS_FILE, categorias)
        # itens_exemplares: convert keys and values already JSON-serializáveis
...
def carregar_todos():
    ...
    if os.path.exists(USUARIOS_FILE) and os.path.exists(CATEGORIAS_FILE) and os.path.exists(ITENS_FILE) and os.path.exists(HISTORICO_FILE):
        try:
            ...
            # recomputar _item_group_counter a partir das chaves
            _item_group_counter = defaultdict(int)
            for chave in itens_exemplares.keys():
                # chave formato: "NOME | 0001-0001"
                try:
    ...
    # se não existir ou falhar, inicializar com padrão
    usuarios = _default_usuarios.copy()
    categorias = _default_categorias.copy()
...
# -------------------------
# Helpers para scroll com mouse wheel
# -------------------------
def _bind_mousewheel(widget):
    # works for Listbox and Treeview
    ...
    # bind according to platform
    widget.bind("<Enter>", lambda e: _setup_bindings(widget))
    widget.bind("<Leave>", lambda e: _remove_bindings(widget))

def _setup_bindings(widget):
...
def sys_platform_is_mac():
    return os.name == "posix" and getattr(__import__('sys'), 'platform', '').startswith('darwin')
...
def abrir_itens_categoria(parent, categoria):
    ...
    # add vertical scrollbar and mousewheel support
...
def excluir_grupo_item():
    # EXCLUSÃO DE GRUPO APENAS PARA ACCESSFULL (EXATO)
    if usuario_logado["tipo"] != "ACCESSFULL":
        messagebox.showerror("PERMISSÃO", "APENAS ACCESSFULL PODE EXCLUIR GRUPOS.")
        return
    nome = simpledialog.askstring("EXCLUIR GRUPO", "NOME DO ITEM (GRUPO) PARA EXCLUIR TOTALMENTE:")
    if not nome:
        return
...
def tela_usuarios(parent):
    limpar_frame(parent)
    ttk.Label(parent, text="GERENCIAR USUÁRIOS (ACESSO: ACCESSFULL)", font=("Segoe UI", 12, "bold")).pack(anchor="w", pady=(2,8))
    ...
    def criar_u():
        if usuario_logado["tipo"] != "ACCESSFULL":
            messagebox.showerror("PERMISSÃO", "APENAS ACCESSFULL PODE CRIAR USUÁRIOS.")
            return
        nome = simpledialog.askstring("USUÁRIO", "NOME DO USUÁRIO:")
        senha = simpledialog.askstring("SENHA", "SENHA:")
    ...
    def excluir_u():
        if usuario_logado["tipo"] != "DIRETOR":
            messagebox.showerror("PERMISSÃO", "APENAS DIRETORPODE EXCLUIR USUÁRIOS.")
            return
        sel = listbox.curselection()
        if not sel:
            messagebox.showinfo("SELEÇÃO","ESCOLHA UM USUÁRIO.")
