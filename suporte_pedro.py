...
class DatabaseConnector:
	...
	def __init__(self):
		# Configurações do banco de dados (usando variáveis de ambiente ou valores padrão)
	...
	def connect(self):
		...
		try:
			...
			# Usamos DictCursor para retornar resultados como dicionários, facilitando o acesso por nome da coluna
			...
			self._conn = None # Garante que a conexão está nula em caso de falha
	...
	def execute(self, query: str, params: Tuple[Any, ...] = None, fetch: str = 'all') -> List[Dict] or Dict or int or None:
		...
		try:
			...
			if fetch == 'one':
				...
				# Converte a linha DictRow para um dicionário padrão antes de retornar
				return dict(row) if row else None 
			elif fetch == 'all':
				# Converte todas as linhas DictRow para uma lista de dicionários padrão
			...
			if fetch == 'insert_returning':
				row = self._cursor.fetchone()
				# Retorna o primeiro (e esperado) valor (geralmente o ID)
...
class Dados:
	...
	def atribuir_cargo_ao_usuario(self, id_usuario: int, id_juncao_cargos_permissoes: int) -> int or None:
		...
		Recebe o ID do Usuário e o ID de Junção de Cargos/Permissões.
		Insere o id_juncao_cargos_permissoes na tabela JUNCAO_USUARIOS_CP.
	...
	def realizar_movimentacao(self, executor_id_juncao_cp: int, id_item_movido: int, id_juncao_cp_receptor: int) -> bool:
		...
		# Define id_locais = NULL porque o item não está em um local físico, mas sim com um usuário.
		update_item_query = """
			UPDATE ITENS
		...
		# Nota: id_locais é NULL, pois o item foi movido do local para um usuário.
		...
		# O id_juncao_cp_receptor é usado como o responsável pela movimentação (executor) no log,
		# pois é a pessoa que está recebendo o item (saída do estoque).
	...
	def inserir_nome_item(self, nome_item: str, id_categoria: int) -> int or None:
		...
		# CORREÇÃO: Usar self._db.execute
		...
		"""
		# CORREÇÃO: Usar self._db.execute
		result = self._db.execute(query_insert, (nome_item, id_categoria), fetch='insert_returning')
		
		if result:
			print(f"INFO: Novo tipo de item '{nome_item}' inserido. ID: {result}")
			return result
		return None
	...
	def registrar_patrimonio(self, id_nomes_itens: int, numero_patrimonio: str, id_status_inicial: int) -> int or None:
		...
		inicial do item (Ex: 1 - DISPONÍVEL).
		...
		"""
		# CORREÇÃO: Usar self._db.execute
		result = self._db.execute(query, (id_nomes_itens, numero_patrimonio, id_status_inicial), fetch='insert_returning')
		
		if result:
			print(f"INFO: Item patrimonial '{numero_patrimonio}' inserido. ID: {result}")
			return result
	...
	def registrar_movimentacao_inicial(self, id_itens: int, id_local_destino: int, id_juncao_usuario_cp: int) -> int or None:
		...
		* id_locais é o local de destino, conforme DDL de MOVIMENTACOES.
		...
		# CORREÇÃO: Ajustada a query para usar 'id_locais' em vez de 'id_local_origem' e 'id_local_destino', 
		# conforme o DDL fornecido.
		query = """
			INSERT INTO MOVIMENTACOES (id_itens, id_locais, id_status, id_juncao_usuario_cp, data) 
			VALUES (%s, %s, %s, %s, NOW()) 
			RETURNING id_movimentacoes;
		"""
		params = (id_itens, id_local_destino, STATUS_DISPONIVEL_ID, id_juncao_usuario_cp)
		# CORREÇÃO: Usar self._db.execute
		result = self._db.execute(query, params, fetch='insert_returning')
	...
	def excluir_item(self, id_item: int) -> bool:
		"""
		[Exclusão de Item]
		Remove um item e todo seu histórico.
		ATENÇÃO: Isso exigirá que as FOREIGN KEYs estejam definidas com ON DELETE CASCADE, 
		caso contrário, a exclusão falhará.
		"""
		# 1. Exclui o histórico de movimentações (Se o FK não tiver CASCADE, isso é obrigatório)
		query_movimentacoes = "DELETE FROM MOVIMENTACOES WHERE id_itens = %s;"
		self._db.execute(query_movimentacoes, (id_item,), fetch='none') # Não verifica o retorno, pois pode não haver histórico.

		# 2. Exclui o item da tabela ITENS
		query_itens = "DELETE FROM ITENS WHERE id_itens = %s;"
		sucesso = self._db.execute(query_itens, (id_item,), fetch='none')

		if sucesso:
			print(f"INFO: Item ID {id_item} e seu histórico excluídos com sucesso.")
			return True
		else:
			print(f"ERRO: Falha ao excluir item ID {id_item}.")
			return False
